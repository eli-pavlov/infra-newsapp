#!/bin/bash
# K3s SERVER install (fast path). Heavy bootstrap (helm/ingress/argocd/secrets/apps)
# is delegated to a systemd oneshot: /usr/local/bin/bootstrap-newsapp.sh
set -euo pipefail
exec > /var/log/cloud-init-output.log 2>&1
set -x
trap 'echo "ERROR at line $LINENO: $BASH_COMMAND" >&2' ERR

# --- Vars injected by Terraform (expanded now by templatefile / rendered user-data) ---
...
wait_for_kubeconfig_and_api() {
  echo "Waiting for kubeconfig and API to be ready..."
  local timeout=120
  ...
  while true; do
    ...
    if [ $(( $(date +%s) - start_time )) -gt $timeout ]; then
      echo "❌ Timed out waiting for kubeconfig/API readiness."
...
install_and_enable_bootstrap_unit() {
  echo "Writing /etc/bootstrap-env for runtime, then creating bootstrap oneshot..."

  # Persist the Terraform-injected environment for use by the runtime bootstrap script.
...
EOF
  ...
  # Write the bootstrap script (single-quoted heredoc -> preserved until runtime)
  cat > /usr/local/bin/bootstrap-newsapp.sh <<'BOOTSTRAP'
#!/bin/bash
set -euo pipefail
exec >> /var/log/bootstrap-newsapp.log 2>&1

# Source boot-time env (written by cloud-init)
...
generate_secrets_and_credentials() {
  echo "Sleeping 30s before creating secrets (deterministic wait) ..."
  sleep 30

  echo "Generating DB password and creating Kubernetes secrets..."
  DB_PASSWORD=$(LC_ALL=C tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 32)

  # Try to read ArgoCD admin secret; do not block if not available
  ...
  if [ -n "$ARGO_PASSWORD" ]; then
    echo "ArgoCD admin pwd retrieved."
    # Optionally write to /root/credentials.txt (secure)
...
EOF
    ...
    echo "ArgoCD admin secret not available yet; continuing."
  fi
...
bootstrap_argocd_apps() {
  echo "Applying Argo CD application manifests from ${T_MANIFESTS_REPO_URL}..."
  rm -rf /tmp/manifests || true
  if [ -n "${T_MANIFESTS_REPO_URL:-}" ]; then
    git clone "${T_MANIFESTS_REPO_URL}" /tmp/manifests || true
    /usr/local/bin/kubectl apply -f /tmp/manifests/clusters/dev/apps/ || true
    /usr/local/bin/kubectl apply -f /tmp/manifests/clusters/prod/apps/ || true
  else
    echo "No manifest repo URL; skipping."
  fi
}

# Wait for API reachable before running heavy work (best-effort)
echo "Bootstrap oneshot: waiting for Kube API to become reachable (max 15m)..."
start=$(date +%s)
max_wait=900
while true; do
  if [ -f "${KUBECONFIG}" ] && /usr/local/bin/kubectl get nodes >/dev/null 2>&1; then
    echo "Kube API reachable; continuing bootstrap."
    break
  fi
  if [ $(( $(date +%s) - start )) -gt $max_wait ]; then
    echo "Kube API not reachable after ${max_wait}s; exiting bootstrap with failure."
    exit 1
  fi
  sleep 5
done
...
UNIT
  ...
  # enable+start the unit now; if it fails it's visible in the journal but cloud-init will finish
  systemctl enable --now bootstrap-newsapp.service || systemctl start bootstrap-newsapp.service || true

  echo "Installed and started bootstrap-newsapp.service (logs: /var/log/bootstrap-newsapp.log and journalctl -u bootstrap-newsapp)."
}

# ---------- main (cloud-init fast path) ----------
main() {
  install_base_tools
  get_private_ip
  install_k3s_server
  ...
  # Hand off heavy operations to the systemd oneshot (install_helm onward)
  install_and_enable_bootstrap_unit

  echo "Cloud-init finished fast path. Heavy bootstrap running under systemd oneshot."
}
