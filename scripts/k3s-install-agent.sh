...
# These are rendered by Terraform templatefile()
...
# Default storage label; setup_local_db_volume may set to "oci"
DB_STORAGE_LABEL="local"

# --- Function Definitions ---

# Wait until K3s server API responds
wait_for_server() {
...
# Firewalld can interfere with K3s iptables rules
systemctl disable firewalld --now

# Prepare storage for DB node; supports absence of extra block volume and preserves data migration
setup_local_db_volume() {
  # Only run on DB nodes
  ...
  # Detect attached block volume (OCI path first, then common Linux device names)
  ...
  # --- Pre-mount migration from root → disk (avoid hiding root data by the final mount) ---
  ...
  # Stable mount via UUID at /mnt/oci/db (original mountpoint/options)
...
main() {
  ...
  wait_for_server
  setup_local_db_volume
  # Append discovered storage kind to node labels (local/oci)
